{ { config(materialized = 'table') } } ---------------Generates the Data Cube for a Single Store (ID = 'AAAAAAAAAABAAAAA'). Vary 'rank' to generate for other stores(alphabetical)----------------------------------------------------------------
with profit_by_store_dly_abstracted as (
  with profit_by_store_dly_abstracted_temp as (
    select
      ...
      NETPROFIT,
      rank() over (
        partition by SOLDDATE
        order by
          STOREID
      ) as rank
    from
      "DEMO_DB"."PUBLIC"."PROFIT_BY_STORE_DLY"
  )
  select
    *
  from
    profit_by_store_dly_abstracted_temp
  where
    rank = 1
),
profit_by_store_wkly_abstracted as (
  with profit_by_store_wkly_abstracted_temp as (
    ...
    from
      "DEMO_DB"."PUBLIC"."PROFIT_BY_STORE_WKLY"
  )
  select
    *
  from
    profit_by_store_wkly_abstracted_temp
  where
    rank = 1
),
sale_by_store_dly_abstracted as (
  with sale_by_store_dly_abstracted_temp as (
    ...
    from
      "DEMO_DB"."PUBLIC"."SALE_BY_STORE_DLY"
  )
  select
    *
  from
    sale_by_store_dly_abstracted_temp
  where
    rank = 1
),
sale_by_store_wkly_abstracted as (
  with sale_by_store_wkly_abstracted_temp as (
    ...
    from
      "DEMO_DB"."PUBLIC"."SALE_BY_STORE_WKLY"
  )
  select
    *
  from
    sale_by_store_wkly_abstracted_temp
  where
    rank = 1
),
date_rank as (
...
profit_sale_wkly_wkly as (
  with profit_sale_wkly as (
    select
      ...
      b.SALECOUNT
    from
      profit_by_store_wkly_abstracted a
      inner join sale_by_store_wkly_abstracted b on a.WEEKID = b.WEEKID
...
profit_sale_dly_wkly as (
  with profit_sale_dly as (
    select
      ...
      b.SALECOUNT
    from
      profit_by_store_dly_abstracted a
      inner join sale_by_store_dly_abstracted b on a.SOLDDATE = b.SOLDDATE
      and a.STOREID = b.STOREID
  )
  select
    b.D_DATE_SK,
